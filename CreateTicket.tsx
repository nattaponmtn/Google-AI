...
import { Camera, Upload, BrainCircuit, Loader2 } from 'lucide-react';
...
export const CreateTicket: React.FC<CreateTicketProps> = ({ companies, assets, onCreate, onCancel }) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        const analysis = await analyzeMaintenanceImage(base64Data);
        
        setTitle(analysis.title);
        setDescription(analysis.description);
        
        // Map AI priority string to Enum
        const prioMap: {[key: string]: Priority} = {
            'Low': Priority.LOW,
            'Medium': Priority.MEDIUM,
            'High': Priority.HIGH,
            'Critical': Priority.CRITICAL
        };
        if (prioMap[analysis.priority]) {
            setPriority(prioMap[analysis.priority]);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        alert("Failed to create ticket. Please check console and try again.");
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
      <div className="p-6 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
        ...
        <div className="bg-blue-100 text-blue-800 text-xs px-3 py-1 rounded-full flex items-center gap-1">
        ...
        <div className="bg-slate-50 p-6 rounded-xl border-2 border-dashed border-slate-300 text-center relative group hover:border-blue-400 transition-colors">
          ...
          {previewImage ? (
              ...
              {isAnalyzing && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded-lg">
                  <div className="text-white flex flex-col items-center">
                    ...
                    <span>Gemini is analyzing equipment...</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col items-center text-slate-500">
              ...
              <p className="font-medium text-slate-700">Click to upload photo</p>
              <p className="text-xs mt-1">Upload an equipment photo to auto-fill details</p>
            </div>
          )}
        </div>
