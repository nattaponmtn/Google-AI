...
import { User, Shield, Bell, LogOut, Building2, RefreshCw, Loader2 } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ currentUser, onLogout, onSync }) => {
  const [isSyncing, setIsSyncing] = useState(false);

  const handleSync = async () => {
    setIsSyncing(true);
    await onSync();
    setTimeout(() => setIsSyncing(false), 1000); // Min duration for visual feedback
  };
        ...
        <div className="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-500">Manually refresh data from server</p>
                </div>
            </div>
            <button 
                onClick={handleSync}
                disabled={isSyncing}
                className="flex items-center gap-2 text-xs font-bold text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded transition-colors disabled:opacity-50"
            >
