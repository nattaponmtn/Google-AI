...
export const WorkOrderDetails: React.FC<WorkOrderDetailsProps> = ({ 
  ...
  const stockErrors = partsUsed.map(part => {
    const partInfo = inventoryParts.find(p => p.id === part.partId);
    if (!partInfo) return null;
    
    const original = initialPartsUsed.find(p => p.id === part.id);
    const originalQty = original ? original.quantityUsed : 0;
    
    const delta = part.quantityUsed - originalQty;
    
    if (delta > partInfo.stockQuantity) {
        return { 
            id: part.id, 
            msg: `Stock Insufficient! Available: ${partInfo.stockQuantity}, Adding: ${delta}` 
        };
    }
    return null;
  }).filter(Boolean);

  const isAnyStockError = stockErrors.length > 0;

  const handleAddPart = (partId: string) => {
      if (!partId) return;
      
      const partInfo = inventoryParts.find(p => p.id === partId);
      if (!partInfo) return;
      ...
      if (existing) {
          handlePartQuantityChange(existing.id, existing.quantityUsed + 1);
  ...
  const handlePartQuantityChange = (id: string, newQty: number) => {
      setPartsUsed(prev => prev.map(p => {
          if (p.id !== id) return p;
          return { ...p, quantityUsed: Math.max(1, newQty) };
      }));
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-32 md:pb-10 relative">
        ...
        <div className="flex-1">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <button 
                    onClick={handleSave}
                    disabled={isSaving || isDeleting || isAnyStockError}
                    className="hidden md:flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                            ...
                            <div className="flex-1">
                                ...
                                {isEditable ? (
                                    ...
                                    >
                                        ...
                                        {systems
                                            .filter(s => !currentCompanyId || s.companyId === currentCompanyId)
                                            .map(s => <option key={s.id} value={s.id}>{s.name}</option>)
                                        }
                                    </select>
                                ) : (
                                    <p className="font-medium">{systems.find(s => s.id === currentSystemId)?.name || currentSystemId || '-'}</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3 text-slate-700">
                            ...
                            <div className="flex-1">
                                ...
                                {isEditable ? (
                                    ...
                                    <p className="font-medium">{equipmentTypes.find(e => e.id === currentEquipmentTypeId)?.name || currentEquipmentTypeId || '-'}</p>
                                )}
                            </div>
                        </div>
                    </div>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="space-y-3">
                    {partsUsed.map(part => {
                        const partInfo = inventoryParts.find(p => p.id === part.partId);
                        const error = stockErrors.find(e => e?.id === part.id);
                        
                        return (
                            <div key={part.id} className={`p-3 rounded-lg border flex flex-col gap-2 ${error ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-100'}`}>
                                <div className="flex justify-between items-start">
                                    <div className="flex-1 min-w-0 mr-2">
                                        <p className="text-sm font-medium text-slate-800 truncate">{partInfo?.name || part.partId}</p>
                                        <p className="text-xs text-slate-400">In Stock: {partInfo?.stockQuantity || 0}</p>
                                    </div>
                                    <button onClick={() => handleRemovePart(part.id)} className="p-1 text-red-400 hover:text-red-600"><Trash2 size={14} /></button>
                                </div>
                                
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-1 bg-white rounded border border-slate-200">
                                        <button 
                                            onClick={() => handlePartQuantityChange(part.id, part.quantityUsed - 1)} 
                                            className="p-1 px-2 text-slate-500 hover:bg-slate-50 border-r border-slate-100"
                                        >
                                            <Minus size={12} />
                                        </button>
                                        <input 
                                        ...
                                        <button 
                                            onClick={() => handlePartQuantityChange(part.id, part.quantityUsed + 1)} 
                                            className="p-1 px-2 text-slate-500 hover:bg-slate-50 border-l border-slate-100"
                                        >
                                            <Plus size={12} />
                                        </button>
                                    </div>
                                    {error && (
                                        <span className="text-[10px] font-bold text-red-600 flex items-center gap-1 bg-red-100 px-2 py-0.5 rounded">
                                            <AlertTriangle size={10} /> Over Stock!
                                        </span>
                                    )}
                                </div>
                                {error && <p className="text-xs text-red-500">{error.msg}</p>}
                            </div>
                        );
                    })}
                        ...
                        <div className="p-3 bg-blue-50 rounded-lg border border-blue-100 animate-in fade-in slide-in-from-top-2">
                             ...
                             >
                                 ...
                                 {inventoryParts.map(p => {
                                     const isOutOfStock = p.stockQuantity < 1;
                                     return (
                                         <option key={p.id} value={p.id} disabled={isOutOfStock} className={isOutOfStock ? 'text-red-400 italic' : ''}>
                                             {p.name} {p.nameTh ? `(${p.nameTh})` : ''} — Stock: {p.stockQuantity} {isOutOfStock ? '(Out of Stock)' : ''}
                                         </option>
                                     );
                                 })}
                             </select>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-3 md:hidden z-50 flex gap-3 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
          <button 
              onClick={onBack}
              className="px-4 py-3 bg-slate-100 text-slate-600 rounded-lg font-medium active:bg-slate-200"
          >
              <ArrowLeft size={20} />
          </button>
          <button 
              onClick={handleSave}
              disabled={isSaving || isDeleting || isAnyStockError}
              className="flex-1 bg-blue-600 text-white rounded-lg font-bold shadow-lg shadow-blue-200 flex items-center justify-center gap-2 disabled:opacity-50 active:scale-[0.98] transition-transform"
          >
